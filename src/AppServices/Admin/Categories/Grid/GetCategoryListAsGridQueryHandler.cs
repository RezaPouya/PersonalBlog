...
public class GetCategoryListAsGridQueryHandler(ICategoryRepository categoryRepository)
    : IQueryHandler<GetCategoryListAsGridQuery, GridDataSourceResult<CategoryGridDto>>
{
    public async Task<GridDataSourceResult<CategoryGridDto>> Handle(GetCategoryListAsGridQuery input,
        CancellationToken cancellationToken)
    {
        GridDataSourceResult<CategoryGridDto> gridResult = await categoryRepository.GetCategoryGridAsync(input, cancellationToken);
